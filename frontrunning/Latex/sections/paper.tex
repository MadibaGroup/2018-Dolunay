...
The history of the \emph{front-running} goes back to the ancient world, where markets in financial instruments were first started. For a trade to take place in those times, all the parties were required to be physically available in the exchange office. Clearly in such scenarios front-running was possible yet not detectable as a damaging activity. However with the rapid advances of the technology in the last few years, trading is not happening in person anymore and a greater amount of applications, whether financial or non-financial, are now vulnerable to front-running. Furthermore, Blockchain technology has introduced a new form of front-running which is inherent in all of the decentral applications. The other two issues which orbit around the front-running, are arbitrage and censorship. However, the goal of this paper is to provide a complete history of the front-running issue and thus the aforemationed will remain out of the scope of this research study. As there has been little quantitative analysis and research of the front-running issue, whether traditional or the new ones on that happen in decentralized application, we aim to take a thorough look into the history of the front-running, systematize all the solutions that have been proposed to this issue so far, and eventually match these solutions to their suitable use cases.\footnote{A note}
...
%TODO:  do this on top 10 Dapps, based on gas usage or a website. we wanted to look through the popular dapps but there's no such a list . therefor we used different examples as we could came up with. 
%maybe look into cryptokitties breed() if it is possible to front run any section
%possible : maybe top 25 active daily usage DApps from dappradar.com 
...
%TODO:  do this on top 10 Dapps, based on gas usage or a website. we wanted to look through the popular dapps but there's no such a list . therefor we used different examples as we could came up with. 
%maybe look into cryptokitties breed() if it is possible to front run any section
%possible : maybe top 25 active daily usage DApps from https://dappradar.com/dapps 
...
\subsection{Historical evidence in the Blockchain}  % JEREMY's COMMENT:  try to sell it in a way that that the audience know that there are 1000 of the applications prone to the frontrunning but we picked 3 because they are different.
...
%TODO: talk about why these three. just for emprical /theorical examples, there are tons of examples of front-running, here's three that we chose that are way different. one is emprical that we had to build tools to 
