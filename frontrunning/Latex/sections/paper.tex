...
Nowadays, there are a wide range of decentral applications implemented on the blockchain. Built on the blockchain infrastructure, there is no trusted third party within any of the decentralized applications and there is no central version of information that is vulnerable to security attacks. However, due to the inherent nature of the blockchain data structure, these applications are open to what is called \emph{front-running}. One such application is \textbf{Bancor} -- an Ethereum-based application that allows users to exchange their tokens without any counter-party risk. Researchers have shown that this financial blcokchain application is open to a simple front-running attack.  However, front-running vulnerability is not limited to the financial application and it can happen in other types of blockchain applications as well. 
...
Although this incident does not involve transaction reordering in the blocks, it shows how miners can modify their code to behave in a certain way to front-run other transactions and take advantage of the monetary profit. The dataset used for this analysis can be found on our GitHub repository.\footnote{Frontrunning Status.im ICO dataset: \url{http://github.com/TODO:ANONYMOUS}}
