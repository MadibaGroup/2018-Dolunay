...
Two strategies: (1) Attacker knows the domain name, then he makes sure he is first to reveal. (2) Attacker is speculating, so he waits to see other reveal, then insert myself with the information from the other reveal. (3) If my amount is bigger, I see other reveal and then reveal. 

Displacement attack <-- 

A front-runner using the leaked information can bid with the same amount and reveal prior to the original bidder to win the auction~\cite{ENSHandlingFrontRunningDiscussion}. The auction model is similar to open auctions in \ref{sec:cryptogames}.
