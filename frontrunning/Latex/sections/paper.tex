...
%Some edits required here! @mahsa TODO
...
Due to deterministic nature of blockchains, running gambling games based on randomness is close to impossible. There had been attempts to use block headers or such as the seed to randomness but almost all have failed as every data accessible to the smart contract at the time of execution, is also accessible to other smart contracts and they could predict the output on the run. Thus most of the current gambling games on Ethereum, rely on the human factor and network behavior rather than pure randomness. Because of these many infamous gambling games are now running on Ethereum network and front-running them requires a highly sophisticated methods.\\
...
Loopring is an open protocol proposed by Wang~\etal to build decentralized exchanges~\cite{wang2018loopring}. This protocol operates a hybrid model with off-chain order management and communication and merely the order matching takes place on the chain. Using Loopring functionality, the users do not need to host their assets on the exchange and and instead they keep their assets in their blockchain wallet. For example, if a user A wants to exchange \emph{token A } to \emph{token B}, she keeps \emph{token A }  safe in her wallet and the Loopring protocol automatically broadcasts the order to the network after user's authorization. If user B wants to exchange, there would be match making miners on the Loopring protocol who matches the orders and swaps the tokens through a smart contract. To solve the front-running issue within the decentralized exchanges, Loopring protocol proposes \emph{dual-authoring} scheme. This solution set two levels of authorization for orders - one for settlement, and one for order matching. In summary, in this scheme a wallet generates a pair of public key (auth-address) and private key (auth-key) for the user, these keys are different from the user's address and its corresponding private key. The address of order's owner, order parameters, and the auth-address are hashed. The hash value, which is called the order-hashed then is signed using the owner's private key. The signature of the ring-hash together with the auth-key are sent to the  miner for matching. In this stage, orders are added to various rings; 
