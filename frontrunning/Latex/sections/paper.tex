...
Given the purpose of this entire paper is systemizing the existing literature, we do not re-enumerate the literature here. However, we note two things. First, we are not aware of any other systematic study of this issue. Second, front-running is related to two well-studied concepts: double-spending and rushing adversaries~\cite{kosba2016hawk}.
...
Another example is in the design of ERC20 standard~\cite{erc20}, is on the allowance functionality. \textit{approve()} function in the specification allows a second entity to be able to spend N tokens from the sender's balance. In order to change the allowance, sender must set the new allowance value to\textit{approve()}. An attacker, using the insertion attack, could front-run the new allowance transaction and spend the old value before the new value is set~\cite{blockchainProjectAllowanceAttack,erc20GithubIssue}. Later on solutions such as \textit{decreaseApproval() / increaseApproval()} was added instead of replacing the value.
