...
%\textbf{Note to Change the structure of the paper: to pitch this paper as a case study, we can polish it as a status,im ico case study: 1) here is the status.im ico project that we look at 2) we find that the issue is mostly the frontrunning 3) So we take a deep look into what is frontrunning 4) we find other case studies that are vulnerable and go through each case}
...
\subsection{Traditional Front-running} %Definition of front-running
...
%TODO: frontrunning categories: ICOs, non-ICOs  DApps. why we chose these, top recent activities.

%______________________________________________________________%


%TODO: but we missed ICOs becasue they are high intensity usage in short period of time and does not include in the list of the most used ones.
%
...
%justify why started with ICO and go to other apps . refrence to ICO section for actual justification
...
\section{Cases of Front-running in ICOs} 
%icos are easy to be missed in the overall most used apps. 
%Make subsections here. one for ico's and why they are important and not in the most used daaps ones. one for  status ico, and different ones for each part
%justify why ICOs were missed in the previous section. argue why 100 other things like ICOs that were missing from the list . ICOs were missed becasue no ico was happening there, and also the website filteres through known applications, we could not think of any other scenarios that are similar to icos.
