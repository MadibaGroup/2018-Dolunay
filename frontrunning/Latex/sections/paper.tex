...
%similar to market, ens cause they are auctions. generalizing to auction front-running. auction-front-running
%exapand the front-running attack...


% refer from ens stuff back here
...
%too dense. need more explanation. --> explain what sealed auction is? whatsealed mean, refer to cryptokitties. what is actually bein auctions who deposits and what.  how ens actually works . 
% two ways to add bid to an auction, start an auction and bid. but otehr method is that. whta sealed mean, domain is sealed but not anything else.  ENS auction details and then say . 
%same price the first transaction would win, front-running. 
%explain more on ENS auction
...
\caption{\scriptsize The percentage of Ethereum blocks mined between block 3903900 and 3908029, this is the time frame in which Status.im ICO was running. This percentage roughly shows the hashing power ratio each miner had at that time. \label{fig:mining_pool_ratio}} % IS THIS STATEMENT CORRECT?
\end{figure}
...
\item \emph{Disincentivizing Front-running Actors:} The call market solution also pivots profitable gains that front-running miners stand to gain into fees that they collect~\cite{clark2014decentralizing}, removing the financial incentive to front-run. To the same end, Malinova~\cite{malinova2017market} propose a design for a decentralized exchange which uses economic models in their trading market design to make front-running expensive and more costly than profitable. \textblue{this is what it does, how does it do it?}
% make it more obvious how they do this. how they changed it


%find a home for localethereum. explain the scenario where this actually works, how it can be front-run in original scenario, how this prevents front-running.
\item \emph{\textblue{Technique}:} LocalEthereum, an escrow-based exchange, uses challenge random values from the website to be included in the signature of the transaction (See Code ~\ref{code:localethereum} in Appendix~\ref{app:code}), hence removing any possibility of replay attack or the possibility of any other address modifying key information in the order. \textblue{how does this solve front-running?}
